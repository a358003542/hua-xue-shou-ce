...
原书Section 1 的内容已经丢弃了。

Section 1大部分内容是介绍有机化合物的IUPAC命名方法的，这块推荐读者找专门的有机化学书籍或者干脆直接去阅读IUPAC标准颜色书籍。

后半部分关于有机化合物的物理常数的表格已经遗失了，有机化合物种类实在太多了。兰氏化学手册第十三版就因为这块内容占据了全书三分之一的内容（大约六百多页），甚至为了让查询这个表格方便又额外增加了一百页的索引表。就算是这样罗列的也可能只是有机化合物的冰山一角，而且查询起来很不方便。显然有机化合物的信息含量已经超出了这种传统的书籍制表法的承载能力。如果有需求推荐查阅相关有机化合物物性数据库，比如：NIST数据库，pubchem数据库等等。
